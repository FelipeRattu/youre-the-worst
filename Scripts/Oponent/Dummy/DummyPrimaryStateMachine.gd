...
var icon = preload("res://Sprites/Oponents/Dummy/DummyIcon.png")
...
func _enter_state(new_state, old_state):
	match new_state:
		states.Idle:
			primaryAnimationPlayer.play("DummyIdle")
		states.Run:
			parent.canCrouch = true
			primaryAnimationPlayer.play("DummyRun")
		states.Slide:
			parent.canCrouch = true
			primaryAnimationPlayer.play("DummySlide")
		states.Jump:
			parent.weight = parent.weightUp
			parent.jump()
			primaryAnimationPlayer.play("DummyJump")
		states.Fall:
			parent.weight = parent.weightDown
			primaryAnimationPlayer.play("DummyFall")
		states.Finish:
			parent.get_node("Roll").stop()
			parent.canCrouch = false
			match GameManager.causeOfGameOver:
				"Win": primaryAnimationPlayer.play("DummyLose")
				"Lose": primaryAnimationPlayer.play("DummyWin")
