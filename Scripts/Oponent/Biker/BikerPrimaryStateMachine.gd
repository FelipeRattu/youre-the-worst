...
var icon = preload("res://Sprites/Oponents/Biker/BikerIcon.png")
...
func _enter_state(new_state, old_state):
	match new_state:
		states.Idle:
			primaryAnimationPlayer.play("BikerIdle")
		states.Run:
			parent.canCrouch = true
			primaryAnimationPlayer.play("BikerRun")
		states.Slide:
			parent.canCrouch = true
			primaryAnimationPlayer.play("BikerSlide")
		states.Jump:
			parent.weight = parent.weightUp
			parent.jump()
			primaryAnimationPlayer.play("BikerJump")
		states.Fall:
			parent.weight = parent.weightDown
			primaryAnimationPlayer.play("BikerFall")
		states.Finish:
			parent.get_node("Roll").stop()
			parent.canCrouch = false
			match GameManager.causeOfGameOver:
				"Win": primaryAnimationPlayer.play("BikerLose")
				"Lose": primaryAnimationPlayer.play("BikerWin")
